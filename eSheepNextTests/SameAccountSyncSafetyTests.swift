...
final class SameAccountSyncSafetyTests: XCTestCase {
    ...
    func testRecoveryEngineRetryPreservesPartialIncrementalCursor() {
        XCTAssertFalse(CloudSyncActor.shouldResumeRecoveryEngineState(
            after: "engineResetPending"
        ))
        XCTAssertFalse(CloudSyncActor.shouldResumeRecoveryEngineState(
            after: "accountReviewCatchUp"
        ))
        XCTAssertTrue(CloudSyncActor.shouldResumeRecoveryEngineState(
            after: "engineResetInProgress"
        ))
        XCTAssertTrue(CloudSyncActor.shouldResumeRecoveryEngineState(
            after: "engineResetFailed"
        ))
        XCTAssertTrue(CloudSyncActor.shouldResumeRecoveryEngineState(
            after: "accountReviewEngineResetInProgress"
        ))
        XCTAssertTrue(CloudSyncActor.shouldResumeRecoveryEngineState(
            after: "accountReviewEngineResetFailed"
        ))
    }

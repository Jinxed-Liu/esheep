...
- CloudBase Gateway 16 项自动化测试、类型/语法检查与生产依赖审计通过；高危/严重漏洞为零。旧 Identity Worker 15 项协议回归测试继续通过，但不属于生产运行链路。
...
- [ ] 在正式非 Beta Xcode 上执行全部 128 项 XCTest（2026-07-20 已在 iPhone 16 Pro 使用 Beta Xcode 实跑 128/128，不能替代正式 Xcode 门禁）。

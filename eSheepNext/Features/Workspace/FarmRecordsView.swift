...
struct ReproductionEntryView: View {
    @Environment(\.dismiss) private var dismiss
    @Environment(\.modelContext) private var modelContext
    @Query(sort: \SheepRecord.earTag) private var sheep: [SheepRecord]
    @Query(sort: \SemenRecord.code) private var semenRecords: [SemenRecord]
    let account: AccountProfile
    let farm: FarmRecord
    private let commandService = FarmCommandService()
    @State private var eweID: UUID?
    @State private var sireID: UUID?
    @State private var kind = ReproductionRecordKind.breeding
    @State private var occurredAt = Date.now
    @State private var semenName = ""
    @State private var semenID: UUID?
    @State private var result = ""
    @State private var lambCount = 1
    @State private var parity = 1
    @State private var birthDeadCount = 0
    @State private var offspring = [LambingOffspringFormRow()]
    @State private var note = ""
    @State private var errorMessage: String?

    private var ewes: [SheepRecord] { sheep.filter { $0.farmID == farm.id && $0.deletedAt == nil && $0.status == .active && $0.sex == .ewe } }
    private var rams: [SheepRecord] { sheep.filter { $0.farmID == farm.id && $0.deletedAt == nil && $0.status == .active && $0.sex == .ram } }
    private var farmSemen: [SemenRecord] { semenRecords.filter { $0.farmID == farm.id && $0.deletedAt == nil } }

    var body: some View {
        Form {
            Picker("类型", selection: $kind) { ForEach(ReproductionRecordKind.allCases, id: \.self) { Text($0.displayName).tag($0) } }
            Section("对象") {
                Picker("母羊", selection: $eweID) { Text("请选择").tag(UUID?.none); ForEach(ewes, id: \.id) { Text($0.earTag).tag(UUID?.some($0.id)) } }
                if kind == .breeding {
                    Picker("公羊（本交）", selection: $sireID) { Text("使用冻精或未知").tag(UUID?.none); ForEach(rams, id: \.id) { Text($0.earTag).tag(UUID?.some($0.id)) } }
                    Picker("冻精（如适用）", selection: $semenID) { Text("不使用冻精").tag(UUID?.none); ForEach(farmSemen, id: \.id) { Text($0.code).tag(UUID?.some($0.id)) } }
                    Text("本交可以不单独建配种事实；孕检不确认父本。")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                }
            }
            DatePicker("发生时间", selection: $occurredAt)
            if kind == .pregnancyCheck { TextField("结果", text: $result) }
            if kind == .lambing {
                Stepper("胎次：\(parity)", value: $parity, in: 1...20)
                Stepper("产羔数：\(lambCount)", value: $lambCount, in: 1...10)
                Stepper("死胎数：\(birthDeadCount)", value: $birthDeadCount, in: 0...lambCount)
                Section("逐只羔羊") {
                    ForEach($offspring) { $detail in
                        VStack(alignment: .leading, spacing: 8) {
                            Text("羔羊 \(offspring.firstIndex(where: { $0.id == detail.id })!.advanced(by: 1))")
                                .font(.subheadline.weight(.medium))
                            TextField("耳号", text: $detail.earTag)
                            Picker("性别", selection: $detail.sex) {
                                ForEach(LambSex.allCases, id: \.self) { sex in
                                    Text(sex.displayName).tag(sex)
                                }
                            }
                            TextField("初生重（千克）", text: $detail.birthWeightText)
        ...
        .onChange(of: lambCount) { _, _ in
            synchronizeOffspringRows()
        }
        .recordErrorAlert($errorMessage)
    }

    private func save() {
        guard let eweID else { errorMessage = "请选择母羊。"; return }
        do {
            let selectedSemen = kind == .breeding ? farmSemen.first(where: { $0.id == semenID })?.code ?? semenName : nil
            let selectedSire = kind == .breeding ? sireID : nil
            let offspringDrafts = kind == .lambing ? offspring.map { detail in
                LambingOffspringDraft(id: detail.id, earTag: detail.earTag, sex: detail.sex, birthWeightText: detail.birthWeightText)
            } : []
            try commandService.execute(.recordReproduction(eweID: eweID, kind: kind, occurredAt: occurredAt, sireID: selectedSire, semenName: selectedSemen, result: result, lambCount: kind == .lambing ? lambCount : 0, parity: kind == .lambing ? parity : nil, birthDeadCount: kind == .lambing ? birthDeadCount : nil, offspring: offspringDrafts, note: note), in: FarmContext(accountID: account.effectiveAccountID, farmID: farm.id, role: farm.role), context: modelContext)
            dismiss()
        } catch { errorMessage = error.localizedDescription }
    }

    private func synchronizeOffspringRows() {
        if offspring.count < lambCount {
            offspring.append(contentsOf: repeatElement(LambingOffspringFormRow(), count: lambCount - offspring.count))
        } else if offspring.count > lambCount {
            offspring.removeLast(offspring.count - lambCount)
        }
    }
}

private struct LambingOffspringFormRow: Identifiable {
    let id = UUID()
    var earTag = ""
    var sex = LambSex.male
    var birthWeightText = ""
}

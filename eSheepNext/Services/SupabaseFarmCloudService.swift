...
final class SupabaseFarmJoinService {
    ...
    func redeemAndInstall(
        ...
        let redemption = try await SupabaseFarmInviteClient(client: client)
            .redeem(code: code)
        let restoredFarmIDs = try await SupabaseFarmAccessRecoveryService(
            client: client
        ).discoverAndRestoreAccessibleFarms(
            accountID: accountID,
            context: context
        )
        guard restoredFarmIDs.contains(redemption.farmID),
              let farm = try context.fetch(FetchDescriptor<FarmRecord>())
                .first(where: { $0.id == redemption.farmID }),
              farm.role == redemption.role else {
            throw SupabaseFarmCloudError.malformedResponse
        }
        CloudRuntimeNotification.postSyncWake(farmID: redemption.farmID)
        return farm
    }
}

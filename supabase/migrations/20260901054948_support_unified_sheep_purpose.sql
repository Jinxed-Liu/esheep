-- Keep the server-side pedigree qualification projection aligned with the
-- unified purpose command. Existing operation history and stored sheep
-- purposes are left untouched; this only changes how future operations are
-- projected.
...
begin
    ...
    if v_kind = 'care' and
       p_payload #> '{careCommand,setSheepPurpose}' is not null then
        v_command := p_payload #> '{careCommand,setSheepPurpose}';
        v_sheep_id := (v_command ->> 'sheepID')::uuid;
        v_active := v_command ->> 'purpose' = '种公羊';
